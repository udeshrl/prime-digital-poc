...
var subtractionWidget = (function (o) {
    ...
    var defaultSetting = {
        ...
        isGridToggle: true,
        isCarryAllow: true,
        isCarryAnswerAvailable: true,
        alignCarrySuperscript1: "center",
        alignCarrySuperscript2: "center",
    ...
    var model = Backbone.Model.extend({
        ...
        check: function () {
            ...
            for (var i = 0; i < uA.carryAnswerField.toString().length; i++) {
                if (uA.carryAnswerField.toString().split(",")[i] !== '') {
                    count++;
                }
            }
            ...
            if (count == 0) {
                ...
                return uA.carryAnswerField.toString() == aA.carryAnswerField.toString() && uA.carryField.toString() == aA.carryField.toString() && uA.answerField.toString() == aA.answerField.toString();
            }
    ...
    var view = Backbone.View.extend({
        initialize: function (options) {
            ...
            o.model.on("change:isCarryAnswerAvailable", o.toggleCarryAnswer.bind(o));
            o.model.on("change:alignCarrySuperscript1", o.alignCarrySuperscript1.bind(o));
            o.model.on("change:alignCarrySuperscript2", o.alignCarrySuperscript2.bind(o));
        ...
        fontSize: function () {
            ...
            this.el.find("input.carryAnswerField").css("font-size", carryFontSize + 'pt');
        ...
        toggleFullAbbreviationAllowed: function () {
            ...
            if (this.model.get("isFullAbbreviationAllowed")) {
                ...
                this.el.find("input.carryAnswerField").css("width", "91px");
                ...
                this.el.find("input.carryAnswerField").css("width", "41px");
            ...
            this.el.find("input.carryAnswerField").css("width", colWidth + "px");
        ...
        revealAnswer: function () {
            ...
            if (this.model.get("isCarryAnswerAvailable")) {
                if (answers.carryAnswerField) {
                    var carryAnswerList = this.el.find(".carryAnswerField");
                    var carryAns = answers.carryAnswerField;
                    for (var i = 0; i < carryAns.length; i++) {
                        carryAnswerList[i].value = carryAns[i];
                    }
                }
            }
    ...
    var uiSetting = {
        ...
        createGrid: function (el, data) {
            ...
            var carryAnswerRow = 1, carryRow = 1, UnitRow = 1, AnswerRow = 1, symbolCol = 1, extraCol = 1, decimalCol = 0;
            ...
            var temp = [];
            var dec = 0;
            var dollarExists = 0, dollarColumn = 0, centColumn = 0, centExists = 0;
            ...
            var noOfRow = data.rowCount = qLIst.length + carryAnswerRow + carryRow + UnitRow + AnswerRow;
            /*var noOfColumn = data.columnCount = _.max(_.map(qLIst, function (q) {
             return q.length;
             })) + symbolCol;*/

            var noOfColumn = data.columnCount = maxWhole + maxDecimal + symbolCol + extraCol + decimalCol + dollarColumn + centColumn;
    ...
    popupManager = {
        popupInitialSetting: {
            popId: 'subtractionWidget_pop_singleton',
            labels: ["Type", "Answer", "Left", "Top", "Height", "Width", "Length"],
            inputName: [
                {id: "questionOption", type: "text", label: "Numbers to subtract"},
                ...
                {id: "isCarryAllow", type: "checkbox", value: "true", label: "Show regrouping superscript row 1"},
                {id: "isCarryAnswerAvailable", type: "checkbox", value: "true", label: "Show regrouping superscript row 2"},
                {id: "isleftAlignCarrySuperscript1", type: "radio", name: "alignCarrySuperscript1", groupLabel: "Align superscript row 1", value: "left", label: "Left "},
                {id: "iscenterAlignCarrySuperscript1", type: "radio", name: "alignCarrySuperscript1", value: "center", label: "Center"},
                {id: "isleftAlignCarrySuperscript2", type: "radio", name: "alignCarrySuperscript2", groupLabel: "Align superscript row 2", value: "left", label: "Left "},
                {id: "iscenterAlignCarrySuperscript2", type: "radio", name: "alignCarrySuperscript2", value: "center", label: "Center"},
        ...
        updateWidget: function (e) {
            ...
            if (p.find('#questionOption').val().split(",").length < 2) {
                $("#subtractionWidget_pop_singleton .pop-row:last").after("<div class='pop-row'>Please enter minimum two numbers.</div>");
            } else {
                popupManager.hide();
            }
        },
        ...
        updatePopFields: function (view) {
            ...
            for (var i = 0; i < s.length; i++) {
                if (s[i].id == "left" || s[i].id == "top" || s[i].id == "margin_h_line" || s[i].id == "margin_v_line") {
                    ...
                    var radioVal = setting.get(s[i].name);
    ...
    function insertData(el, numberList, dollarExists, tagDollar, centExists, tagCent, rows, columns, symbol, unitLabelAllow, carryAllow, carryAnswerAvailable, fullAbbreviationAllowed, alignCarrySuperscript1, alignCarrySuperscript2, maxDecimalLength) {
        ...
        for (var i = 0; i < numberList.length; i++) {
            ...
            if (dollarExists == 1) {
                if (tagDollar[i] == i) {
                    $(el).find("#r" + l + "c3").append('<div class="digit">$</div>');
                }
            }
            if (centExists == 1) {
                if (tagCent[i] == i) {
        ...
        // insert carryFields()
        for (var i = 0; i <= columns; i++) {
            $(el).find("#r2c" + i).append('<input class="carryField" type="text" maxlength="1" style="text-align:' + alignCarrySuperscript1 + ';" />');
        }

        // insert carryAnswerFields()
        for (var i = 0; i <= columns; i++) {
            $(el).find("#r3c" + i).append('<input class="carryAnswerField" type="text" maxlength="1" style="text-align:' + alignCarrySuperscript2 + ';" />');
        }
        if (centExists == 1) {
        ...
        // insert subtractionSignLabel
        $(el).find("#r" + (rows - 1) + "c1").append('<div class="digit"><span class="subtractDecorator">&#65293;</span></div>');
        ...
        if (!carryAnswerAvailable) {
            $(el).find("#row3").css("display", "none");
        }
    }
    ...
    function getConfigurationWindow(setting, parent) {
        ...
        var a = '<div class="popup-overlay" id="popup-overlay-sub"></div><div id="' + setting.popId + '" class="popup_container">';
        a = a + '<div>';
        var group = false, labelClass;
    ...
    function subtractionWidget(options) {
...
//            	});
                el.find("#row4 .Addgridsquare").click(function () {
                    ...
                    if (a === 0) {
                        jQuery(this).find(".digit").append('<span class="strike">/</span>');
                    }
                    else {
            ...
            /*var p = jQuery("#row3 .Addgridsquare").find('carryAnswerField').length != 0;
             if(!p){
             jQuery(".subtractionWidget #row3 .Addgridsquare").css("height", "20px");
             }
             var q = jQuery("#row2 .Addgridsquare").find('carryField').length != 0;
             if(!q){
             jQuery(".subtractionWidget #row2 .Addgridsquare").css("height", "20px");
             }*/
        }
        init();
        /*
